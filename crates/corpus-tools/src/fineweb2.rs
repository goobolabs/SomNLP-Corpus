//! FineWeb-2 Somali (`HuggingFaceFW/fineweb-2`, config `som_Latn`) downloader.
//!
//! The dataset contains filtered Common Crawl web text. Parquet representations are available on
...
pub const SOURCE_TAG: &str = "fineweb2";
pub const DATASET_NAME: &str = "HuggingFaceFW/fineweb-2";
...
/// Download the Somali FineWeb-2 parquet shards and export one JSONL record per non-empty text row.
pub fn download_fineweb2(
    output: &Path,
    limit: Option<u64>,
    streaming: bool,
) -> Result<crate::Stats> {
    ...
    crate::cli::print_export_summary(
        "FineWeb-2 Somali export complete",
        &stats,
        output,
        &source_url(),
    );
...
mod tests {
    ...
    fn listing() -> Vec<String> {
        vec![
            ...
            "som_Latn/train_extra/0000.parquet".to_string(),
            "som_Latn_Arab/train/0000.parquet".to_string(),
        ]
    }
    ...
    fn excludes_other_language_subsets() {
        ...
        assert!(!shards.iter().any(|s| s.contains("som_Latn_Arab")));
    }
    ...
    fn local_shard_names_are_collision_safe() {
        assert_eq!(
            ...
            "fineweb2_som_Latn_train_0000.parquet"
        );
        assert_ne!(
            local_shard_name("som_Latn/train/0000.parquet"),
