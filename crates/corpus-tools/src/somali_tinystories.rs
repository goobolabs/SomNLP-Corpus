//! Somali TinyStories (`Zyroxx66/somali-tinystories`) downloader.
//!
//! The dataset contains synthetic Somali short stories designed for narrative-language training.
//! Parquet representations are available on [`PARQUET_REVISION`], under `default/train/`.
...
pub const SOURCE_TAG: &str = "somali_tinystories";
pub const DATASET_NAME: &str = "Zyroxx66/somali-tinystories";
...
/// Download the Somali TinyStories parquet shards and export one JSONL record per non-empty story text row.
pub fn download_somali_tinystories(
    output: &Path,
    limit: Option<u64>,
    streaming: bool,
) -> Result<crate::Stats> {
    ...
    for remote_path in &shards {
        ...
        for text in iter_text_column(&local_path, STORY_COLUMN)? {
            if limit.is_some_and(|limit| written >= limit) {
                break;
            }
            let text = text?;
    ...
    crate::cli::print_export_summary(
        "Somali TinyStories export complete",
        &stats,
        output,
        &source_url(),
    );
...
mod tests {
    ...
    fn local_shard_names_are_collision_safe() {
        assert_eq!(
            ...
            "somali_tinystories_default_train_0000.parquet"
        );
        assert_ne!(
            local_shard_name("default/train/0000.parquet"),
    ...
    #[test]
    fn missing_story_column_returns_error() {
        let fields = vec![Field::new("character_role", DataType::Utf8, false)];
        let columns: Vec<ArrayRef> = vec![Arc::new(StringArray::from(vec!["Protagonist"]))];
        let (_temp, path) = create_test_parquet(fields, columns);

        let res: Result<Vec<_>> =
            iter_text_column(&path, STORY_COLUMN).and_then(|iter| iter.collect());
        assert!(res.is_err());
    }

    #[test]
    fn skips_empty_story_rows_and_excludes_metadata() {
        let fields = vec![
            Field::new(STORY_COLUMN, DataType::Utf8, false),
            Field::new("moral", DataType::Utf8, false),
        ...
        let columns: Vec<ArrayRef> = vec![
            ...
            Arc::new(StringArray::from(vec![
                "Moral 1", "Moral 2", "Moral 3", "Moral 4",
            ])),
            Arc::new(StringArray::from(vec![
                "Forest", "River", "Village", "Town",
            ])),
        ];
        let (_temp, path) = create_test_parquet(fields, columns);

        let iter = iter_text_column(&path, STORY_COLUMN).unwrap();
        let stories: Vec<String> = iter
            .filter_map(|r| r.ok())
            .filter(|s| is_non_empty(s))
            .collect();

        assert_eq!(stories, vec!["Maalin waa tii...", "Sheeko labaad..."]);
        for story in &stories {
            assert!(!story.contains("Moral"));
            assert!(!story.contains("Forest"));
            assert!(!story.contains("River"));
            assert!(!story.contains("Village"));
            assert!(!story.contains("Town"));
        }
    }
}
